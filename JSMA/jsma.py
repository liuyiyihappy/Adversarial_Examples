...
def saliency_map(jacobian, target_index, increasing, search_space, nb_features):
    ...
    zero_diagonal = torch.from_numpy(tmp).byte()   # torch.from_numpy(tmp)：把tmp的numpy形式转换为torch（tensor形式：包含单一数据类型元素的多维矩阵）
    ...
    if increasing:
        mask1 = torch.gt(alpha, 0.0).byte()
        mask2 = torch.lt(beta, 0.0).byte()
    else:
        mask1 = torch.lt(alpha, 0.0).byte()
        mask2 = torch.gt(beta, 0.0).byte()
...
def perturbation_single(image, ys_target, theta, gamma, model):
    ...
    var_sample =Variable(torch.from_numpy(copy_sample), requires_grad=True)  #把测试样本特征变成tensor,torch.Size([1, 784])
    outputs = model(var_sample)
    predicted = torch.max(outputs.data, 1)[1]
    print('测试样本扰动前的预测值：{}'.format(predicted[0]))

    var_target = Variable(torch.LongTensor([ys_target,]))   #把对抗样本标签变成一个tensor，torch.Size([1])
    if theta > 0:
        increasing = True   ##
    else:
        increasing = False

    num_features = int(np.prod(copy_sample.shape[1:]))   #np.prod所有元素相乘,copy_sample.shape[1:]：读取矩阵第二维度的长度：(784,)，shape[0]是1.
    shape = var_sample.size()  #shape为[1, 784]
    ...
    if increasing:
        search_domain = torch.lt(var_sample, 0.99) #逐元素比较var_sample和0.99，lt:<则为true
    else:
        search_domain = torch.gt(var_sample, 0.01) #逐元素比较var_sample和0.01，gt:>则为true
    search_domain = search_domain.view(num_features) #resize和num_features一样维度的tensor
    ...
    while (iter < max_iters) and (current[0] != ys_target) and (search_domain.sum() != 0):
        ...
        new_sample = torch.clamp(var_sample_flatten, min=0.0, max=1.0)   #将输入张量每个元素的夹紧到区间 [min,max][min,max]，并返回结果到一个新张量。
        new_sample = new_sample.view(shape)  #shape为[1, 784]
        search_domain[p1] = 0
        search_domain[p2] = 0
...
if __name__=="__main__":
    ...
    #index = 10 # 选择测试样本
    index = 1100
    image = testdata[index][0].resize_(1,784).numpy() # 测试样本特征 ndarray   (1, 784)
